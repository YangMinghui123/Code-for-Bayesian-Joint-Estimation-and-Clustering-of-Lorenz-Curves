...
cbPalette <- c( "#56B4E9", "#009E73", "#F0E442","#D55E00", "#CC79A7")#定义颜色
stateinfo_disjoint_simu1 <- readRDS("D:/Edge Download/state_fips_name.rds")#导入洲名数据
stateinfo_disjoint_simu1$cluster <- as.factor(DahlAns_disjoint1[["zout"]])#根据聚类结果给各洲赋上类别
usmap_disjoint_init <- plot_usmap(data = stateinfo_disjoint_init, values = "cluster", labels = TRUE) +
  scale_fill_manual(values = cbPalette, name = "Cluster")#原始地图
usmap_disjoint_simu1 <- plot_usmap(data = stateinfo_disjoint_simu1, values = "cluster", labels = TRUE) +
  scale_fill_manual(values = cbPalette, name = "Cluster")#根据聚类结果画美国地图
stateinfo_disjoint_simu2 <- readRDS("D:/Edge Download/state_fips_name.rds")#导入洲名数据
stateinfo_disjoint_simu2$cluster <- as.factor(DahlAns_disjoint2[["zout"]])#根据聚类结果给各洲赋上类别
usmap_disjoint_simu2 <- plot_usmap(data = stateinfo_disjoint_simu2, values = "cluster", labels = TRUE) +
  scale_fill_manual(values = cbPalette, name = "Cluster")#根据聚类结果画美国地图
#将3张图放在一起，方便比较
ggarrange(usmap_disjoint_init,usmap_disjoint_simu1,usmap_disjoint_simu2,ncol = 2,nrow =2,widths = c(2,2),heights = c(2,2,2))
