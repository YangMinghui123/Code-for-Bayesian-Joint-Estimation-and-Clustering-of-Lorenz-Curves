...
stateinfo_joint_simu1 <- readRDS("D:/Edge Download/state_fips_name.rds")#导入洲名数据
stateinfo_joint_simu1$cluster <- as.factor(DahlAns_joint1[["zout"]])#根据聚类结果给各洲赋上类别
usmap_joint_init <- plot_usmap(data = stateinfo_joint_init, values = "cluster", labels = TRUE) +
  scale_fill_manual(values = cbPalette, name = "Cluster")#原始地图
usmap_joint_simu1 <- plot_usmap(data = stateinfo_joint_simu1, values = "cluster", labels = TRUE) +
  scale_fill_manual(values = cbPalette, name = "Cluster")#根据聚类结果画美国地图
stateinfo_joint_simu2 <- readRDS("D:/Edge Download/state_fips_name.rds")#导入洲名数据
stateinfo_joint_simu2$cluster <- as.factor(DahlAns_joint2[["zout"]])#根据聚类结果给各洲赋上类别
usmap_joint_simu2 <- plot_usmap(data = stateinfo_joint_simu2, values = "cluster", labels = TRUE) +
  scale_fill_manual(values = cbPalette, name = "Cluster")#根据聚类结果画美国地图
#将3张图放在一起，方便比较
ggarrange(usmap_joint_init,usmap_joint_simu1,usmap_joint_simu2,ncol = 2,nrow =2,widths = c(2,2),heights = c(2,2,2))
